...
def gen_corpus():
    ...
    """
    # 读取数据
    read_path = os.path.join(uppath(2), 'Data', '264_洛炜言情小说全集83本') .encode('utf-8').decode('utf-8')  # used decode
    ...
    with open(save_path + os.sep + 'corpus_luowei_novel.txt', 'w') as fw:
        for i, each in enumerate(dir_list):
            ...
            content_lcut = jieba.lcut(
                # used decode
                content.encode('gbk').decode('gbk').replace(u'\u3000', u'').replace(u'\u300c', u'').replace(u'\u300d', u''))
            # 获得单词的打标
...
def cal_InitStatus():
    ...
    with open(read_path + os.sep + 'corpus_luowei_novel.txt', 'r') as f:
        ...
        count_S, count_B = reduce(lambda m, n: (m[0] + n[0], m[1] + n[1]),
                                  map(lambda x: stop_word(x.encode('utf-8').decode('utf-8')), content))
...
def cal_EmitProbMatrix():
    ...
    with open(read_path + os.sep + 'corpus_luowei_novel.txt') as f:
        content = f.readlines()
        wordsSeries, statusSeries = reduce(lambda n, m: [n[0] + m[0], n[1] + m[1]],
                                           map(lambda x: split_by_tab_emit(x.encode('utf-8').decode('utf-8')), content))  # 获取 单词序列、状态序列
        for status in list('SBME'):
            ...
            wordsSeriesCountLog = dict(
                map(lambda x: (x[0], np.log(x[1] / wordsSeriesCountAll)), wordsSeriesCount.items()))  # used iteritems
...
def main():
    ...
    HMMLgbIns = HMMLgb()
    words = u'''邹进文副校长致开幕辞，他热情地为在座嘉宾回顾了我校及统计学科从峥嵘岁月发展
    至今的渊源与成就，表达了我校及统计学科的开放发展之姿，并恳请大家畅所欲言，为我校统计学科问诊把脉。
    统计与数学学院院长张虎就我校统计学学科发展情况作了汇报。他具体介绍了我校统计学在第四轮学科评估中
    的总体情况、第五轮学科评估工作的进展情况，以及目前该学科在师资、科研、人才培养、社会服务等方面的
    成绩与不足，并从“双一流”建设背景下的学科发展新格局，第五轮学科评估工作的现实需求等角度阐明了学院
    统计学科发展的内在动力与外在要求，也点明了此次论坛的主题。他真诚的希望与会专家为学院统计学科建设、
    经济统计学专业建设多提宝贵意见。
    之后，各位嘉宾纷纷发言，针对学科发展突破口、第五轮学科评估的政策导向与应对策略各抒己见，
    形成了诸多建设性结果。根据议程安排，开幕式结束后设有八场主题学术报告，与会专家分享了最新研究成果，
    并与学院师生开展了深入学术探讨。”
        '''
    # words = u'''这是一个测试！'''
...
if __name__ == '__main__':
    # ss = open('corpus_luowei_novel.txt')
    # print(ss.read())
    main()
    # 最大熵马尔可夫模型 MEMM
